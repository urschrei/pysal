...
Austria}\label{an-illustrative-example-migration-in-austria}
    ...
    Despite being a small toy dataset, the following example is utilizes for
...
in-flows, respectively, and the \textbf{Dij} column is the Euclidian
...
are either being used to describe origin or desitnation attributes
...
done becasue intra-zonal flows are large compared to inter-zonal flows
and would therefore heavily influence the model or becasue it is not
...
were removed for this example. Therefore
...
\(d_ij\). Therefore, we can use an unconstrained gravity model with an
...
increase in distance, if our model predicted 2,500 flows, then we can
...
little to no penalization for model complexity.
...
throug the design matrix starting with 0 as you move through th origin
...
see that the model fit in stronger in the north-west but decrease in the
south-east. Using these patterns, we could then further postulate why
they arise or how we might be able to imrpove model fit.
